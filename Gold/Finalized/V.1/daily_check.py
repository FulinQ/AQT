...
X_test_scaled = scaler_X.transform(X[train_size:])

def create_sequences(X, y, window_size):
    Xs, ys = [], []
    for i in range(len(X) - window_size):
        Xs.append(X[i:(i + window_size)])
        ys.append(y[i + window_size - 1]) # Keep this fix
    return np.array(Xs), np.array(ys)

scaler_y = StandardScaler()
all_horizon_preds = []
test_start_idx = train_size + WINDOW_SIZE - 1
y_test_seqs = []

for horizon_idx, t_col in enumerate(target_cols):
    horizon_days = horizon_idx + 1
    print(f"\n========================================")
    print(f"🎯 Training Models for Horizon {horizon_days} Days ({t_col})")
    print(f"========================================")
    
    MODEL_DIR = f'ensemble_huber_h{horizon_days}'
    os.makedirs(MODEL_DIR, exist_ok=True)
    
    y = df_train_full[t_col].values
    ...
    X_test_seq, y_test_seq_scaled = create_sequences(X_test_scaled, y_test_scaled, WINDOW_SIZE)
    _, y_test_seq_raw = create_sequences(X_test_scaled, y_test, WINDOW_SIZE)

    
    y_test_seqs.append(y_test_seq_raw)
    ...
    for i in range(N_MODELS):
        ...
        model = Sequential([
            ...
            Dense(1) # ทายแยกทีละวัน
        ])

        model.compile(optimizer=Adam(learning_rate=0.001), loss=Huber(), metrics=['mae'])
        
        filename = os.path.join(MODEL_DIR, f'gold_h{horizon_days}_ens_{i}.keras')
    ...
    print(f"Evaluating Ensemble for Horizon {horizon_days}...")
    test_preds = []
    for filename in model_files:
        m = tf.keras.models.load_model(filename)
        test_preds.append(m.predict(X_test_seq, verbose=0))
    
    avg_pred_scaled = np.mean(test_preds, axis=0)
    avg_pred = scaler_y.inverse_transform(avg_pred_scaled.reshape(-1, 1)).flatten()
    all_horizon_preds.append(avg_pred.flatten())
    
# Convert back to Multi-Output format for Evaluation blocks
y_pred_avg = np.column_stack(all_horizon_preds)
y_test_seq = np.column_stack(y_test_seqs)
